    ...
    try {
        ...
        if (!messages || !Array.isArray(messages) || messages.length === 0) {
            return NextResponse.json(
                { success: false, error: "聊天历史为空，无法生成人设" },
                { status: 400 }
            );
        }
        ...
        const userAnswers = messages
            ...
            .map((msg: { content: string }) => msg.content.trim());
        ...
        const brief = `账号类型：${userAnswers[0] || "未说明"}；
性别与生活特征：${userAnswers[1] || "未说明"}；
目标受众：${userAnswers[2] || "未说明"}；
内容领域：${userAnswers[3] || "未说明"}；
平台要求与互动需求：${userAnswers[4] || "未说明"}`;

        // 固定 goal（也可根据需求让用户自定义）
        const goal = "打造一位可复用的 KOS 人设，包含内容方向与表达策略，适配用户指定的平台和受众。";
